...
class FlowHttp extends EventEmitter{
	...
	allowWSRequest(req, fn){
        ...
        if(res){
            ...
            res.writeHead = (statusCode, statusMessage, headers)=>{
                ...
                let sessionCookie = this.buildSesssionCookie(req);
                if(sessionCookie)
	...
	init_socketio_handler(websocketPath, websocketMode) {
		...
		this.websockets = this.io.on('connection', async (conn)=>{
			const jc = FlowHttp.modules.NATS.JSONCodec();
			conn.on('data', (data)=>{
				...
				switch (event) {
					case 'auth::cookie': {
						...
						if (cookie === null) {
							let req = { };
							this.sessionStore.generate(req);
							req.session.user = { token : null };
							session = req.session;
							this.updateSessionRecord(session);
							let sessionCookie = this.buildSesssionCookie(req);
							conn.write(JSON.stringify(['auth::setcookie', {
								cookie: sessionCookie
							}]));
							cookie = sessionCookie;
						}
						this.getSocketSession(cookie).then(sess=>{
							session = sess;
							conn.session = session;
							if(!session.user)
								session.user = { token : null }

							console.log("#### socket:init".redBG, conn.id, session)

							if(session.user.token)
								this.addSocketIdToTokenSocketMap(session.user.token, conn.id);
						}).catch(err=>{
					...
					case 'message': {
						try {
							...
							this.emit(subject, data, { subject, conn, rpc : this });
						}
						catch(ex) {
					...
					case 'publish': {
						...
						if(!this.checkAuth(session.user, subject, data, FlowHttp.METHODS.PUBLISH)) {
							conn.write(JSON.stringify(['publish::response', { rid, error: "Access Denied" }]));
							return;
						}
						...
						conn.write(JSON.stringify(['publish::response', { rid, ack : true }]));
						break;
					}
					case 'unsubscribe': {
						// TODO - sanity checks
						if(!msg || !msg.req) {
							conn.write(JSON.stringify(['unsubscribe::response', { rid, error : 'malformed request' }]));
							return;
						}
						...
						if(!sub) {
							conn.write(JSON.stringify(['unsubscribe::response', { rid, error : 'no such token' }]));
							return;
						}
						...
						conn.write(JSON.stringify(['unsubscribe::response', { rid, ok : true }]));
						break;
					}
					// NATS subscribe
					case 'subscribe': {
						...
						if(!msg || !msg.req || !msg.req.subject) {
							conn.write(JSON.stringify(['subscribe::response', { rid, error : 'malformed request' }]));
							return;
						}
						...
						if(!this.checkAuth(session.user, subject, d_, FlowHttp.METHODS.SUBSCRIBE)) {
							conn.write(JSON.stringify(['subscribe::response', { rid, error: "Access Denied" }]));
							return;
						}

						if(subscriptions.length > NAX_SUBSCRIPTIONS) {
							conn.write(JSON.stringify(['subscribe::response', { rid, error: "Maximum Number of Subscriptions Reached" }]));
							return;
						}
						...
						this.nats.subscribe(subject, (msg, reply, subject, sid) => {
							...
							if(msg.reply) {
								...
								const rid = rids++;
								conn.write(JSON.stringify(['request', { rid, req : { subject, data }}]));
							...
							conn.write(JSON.stringify(['publish', { subject, data }]));
						})
						...
						conn.write(JSON.stringify(['subscribe::response', { rid, token, subject }]));
					...
					case 'request': {
						...
						if(this.messageFilter_ && !this.messageFilter_(subject)) {
							conn.write(JSON.stringify(['response', { rid, error: "Unknown Message" }]));
							return;
						}
						...
						if(!this.checkAuth(session.user, subject, data, FlowHttp.METHODS.REQUEST)) {
							conn.write(JSON.stringify(['response', { rid, error: "Access Denied" }]));
							return;
						}
						...
						this.nats.request(subject, jc.encode(data)).then((msg) => {
							...
							this.handleResponse(conn.id, session.user, subject, data, session);
							...
							conn.write(JSON.stringify(['response', { rid, error }]));
					...
					case 'rpc.req': {
						...
						if(!data)
							data = { };
						if( !this.checkAuth(session.user, subject, data, FlowHttp.METHODS.REQUEST) ){
							conn.write(JSON.stringify(['rpc.resp', {
								rid,
								error: "Access Denied"
							}]));
							return;
							...
							if(!subject || subject == 'init-http') {
								...
								if(listeners.length == 1) {
									let callback = (error, data) => {
										...
										conn.write(JSON.stringify(['rpc::response', {
											rid, error, data
										}]));
									}
			...
			conn.write(JSON.stringify(['auth::getcookie']));

			socketsOpen++;
			let rids = 0;
			this.websocketMap.set(conn.id, conn);
			if(!this.subscriptionMap.has(conn.id))
				this.subscriptionMap.set(conn.id, []);
			const subscriptions = this.subscriptionMap.get(conn.id);

			if(!this.pendingMap.has(conn.id))
				this.pendingMap.set(conn.id, new Map())
			const pending = this.pendingMap.get(conn.id);

			conn.write(JSON.stringify(['message', {
				subject : 'init'
			}]));
			
			this.emit("websocket.connect", {conn});

			conn.on('close', ()=>{
				if(session.user.token) {
					let socket_id_set = this.tokenToSocketMap.get(session.user.token);
					if(socket_id_set) {
						socket_id_set.delete(conn.id);
						if(!socket_id_set.size)
							this.tokenToSocketMap.delete(session.user.token);
					}
				}
				this.websocketMap.delete(conn.id);
				...
				this.subscriptionMap.delete(conn.id);
