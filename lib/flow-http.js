...
class FlowHttp extends EventEmitter{
	...
	constructor(appFolder, options={}){
		...
		this.socketId2StreamMap = new Map();
	}
	...
	initHttp(){
		return new Promise((resolve, reject)=>{
			...
			if(this.config.websocketPath) {
				...
				if(ws) {
					ctx.ws = ws;
					this.io = new ws.Server({ server, path : this.config.websocketPath+'/websocket' });
	...
	initSocketServer(ctx) {
		...
		this.subscriptionsBySubject = new Map();
	...
	initSocketHandler(ctx, websocketMode) {
		...
		this.websockets = this.io.on('connection', async (conn_impl, req)=>{
			...
			console.log("#######conn_impl".red , conn_impl.id)
			...
			socket.on('close', () => {
				...
				console.log("#####socket_id_set closed-1", socket.id)
				...
				if(socket.session.user.token) {
					...
					if(socket_id_set) {
						...
						console.log("#####socket_id_set closed", socket.id, socket_id_set.size)
				...
				while(subscriptions.length) {
					...
					const { token, subject, natsSubscription } = subscription;
					//console.log("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@ REMOVING".brightMagenta, subject,"REFS",refs);
					if(!subscription.sockets.size) {
						//console.log("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@ REMOVING".brightRed, subject,"REFS",refs);
						ctx.subscriptionTokenMap.delete(token);
						this.subscriptionsBySubject.delete(subject);
						natsSubscription.unsubscribe();
					}

					// console.log("+============================================".brightWhite);
					// console.log('ctx.subscriptionTokenMap'.brightGreen, ctx.subscriptionTokenMap.size);
					// console.log('this.subscriptionBySubject'.brightGreen, this.subscriptionsBySubject.size);
				...
				// console.log("SUBSCRIPTIONS:::::::::::::::::::::::::::::::::".brightYellow, subscriptions);
				// console.log("SUBSCRIPTION TOKEN MAP::::::::::::::::::::::::".brightYellow, ctx.subscriptionTokenMap);
				// console.log("SUBSCRIPTION BY SUBJECT:::::::::::::::::::::::".brightYellow, ctx.subscriptionsBySubject);
	...
	handleGRPC(socket, eventName, msg, ctx){
		...
		if(eventName == 'grpc.stream.write'){
			...
			this.writeGRPCStream({
				...
				this.debug && console.log("grpc.stream.response", data, error, response)
				if(error){
	...
	async initGRPC() {
		...
		utils.each(json.services, (value, name)=>{
			this.grpcClients[name] = new proto[name](server,
						grpc.credentials.createInsecure(),this.config.grpc?.options||{});
	...
	writeGRPCStream({client, method, sId, socketId, data}, callback){
		...
		if(typeof data == 'object'){
			streamuid = data.__streamuid__||'';
			delete data.__streamuid__;
		}
		let streamUid = socketId+"_"+streamuid+"_"+method;
		this.debug && console.log("streamUid:", streamUid)
		//console.log("writeGRPCStream", streamUid, data)
		if(!client._streams)
			client._streams = {};
		...
		let next = ()=>{
			...
			socket._sIds.set(streamUid+sId, {method, sId});

			const stream = client._streams[streamUid];
			stream.streamUid = streamUid;
			stream._socketIds.add(socketId);
			stream._callbacks.push(callback);
			stream.write(data);

			let streams = this.socketId2StreamMap.get(socketId)
			if(!streams){
				streams = [];
				this.socketId2StreamMap.set(socketId, streams)
			}
			streams.push(stream);
		}

		if(client._streams[streamUid]){
			next();
			return
		}
		client.waitForReady(this.config.grpc?.clientWaitTime || 5000, (connect_error)=>{

			//gc();console.log(process.memoryUsage())
			//console.log("waitForReady:args", connect_error)
			if(connect_error){
				let socket = this.websocketMap.get(socketId)
				socket?.emit("message", {subject:"grpc.stream.end", data:{sIds:[sId]}})
				return
			...
			client._streams[streamUid] = stream
			stream._callbacks = [];
			stream._id = Date.now();
			...
			stream.on('end', ()=>{
				...
				console.log('stream end', stream._id, streamUid);
				stream._socketIds.forEach(socketId=>{
					let socket = this.websocketMap.get(socketId)
					if(socket){
						let sIds = [...socket._sIds.values()].map(a=>a.sId);
				...
				stream._socketIds = [];
				stream._callbacks = [];
				client._streams[streamUid] = null;

				let streams = this.socketId2StreamMap.get(socketId)
				if(streams){
					streams = streams.filter(s=>s._id!=stream._id);
					this.socketId2StreamMap.set(socketId, streams)
				}
			});
			...
			stream.on('error', (error)=>{
				//console.log('stream error', stream._id, streamUid, error);
				/*
				stream._socketIds.forEach(socketId=>{
					let socket = this.websocketMap.get(socketId)
					if(socket){
	...
	cleanupSocketStreams(socketId){
		let streams = this.socketId2StreamMap.get(socketId)
		//console.log("cleanupSocketStreams", socketId, streams)
		if(!streams)
			return
		...
		else if(ctx.websocketMode == 'NATS')
		{
			switch (event) {
				...
				case 'unsubscribe': {
					...
					let subscriptions = this.subscriptionMap.get(socket.id);
					// subscriptions = subscriptions.filter( sub => {
					// 	return sub.token != token;
					// });
					// this.subscriptionMap.set(socket.id, subscriptions);
					...
					let idx = subscriptions.indexOf(sub);
					if(idx >= 0){
						subscriptions.splice(idx, 1);
					}

					sub.sockets.delete(socket);
					const { natsSubscription, subject } = sub;
				...
				case 'subscribe': {
					...
					//console.log('subscribe msg::::::::::::::::::::::::::::::::::::::::::::::::::'.brightCyan,subject);
					if(subject.length > 256) {
						socket.emit('subscribe::response', { rid, error : 'subject too long' });
						return;
					}
					...
					const subscriptions = this.subscriptionMap.get(socket.id);
					console.log("###### subscribe: socket.id", socket.id, subject, subscriptions.length)
					...
					let subscription = this.subscriptionsBySubject.get(subject);
					if(subscription) {
						subscription.sockets.set(socket, 1)
						//console.log("##################################################".brightYellow);
						//console.log("RE-SUBSCRIBING token",subscription.token,'refs:'.brightMagenta,subscription.refs);
						//console.log('nats relay subscriptions'.brightGreen, subject);
						subscriptions.push(subscription);
						socket.emit('subscribe::response', { rid, subject, token : subscription.token });
						//console.log("##################################################".brightYellow);
						return;
					}
					...
					let token = 'TTT'+FlowUid({ length : 24 });
					const natsSubscription = this.nats.subscribe(subject, { callback : (err, msg) => {
						//this.nats.subscribe(subject, (msg, reply, subject, sid) => {
						// const { reply, data, subject } = msg;
						//console.log('======> processing subject'.brightYellow,subject,'data',typeof(msg.data));
						//const data = data ? jc.decode(msg.data) : null;
						//console.log("-> SUBSCRIPTION",subject);//, data);
						if(msg.reply) {
						...
						//console.log('publishing'.brightGreen,subject,msg.data?jc.decode(msg.data):'NULL');

						const rel = { subject, token };
						if(msg.data)
							rel.data = jc.decode(msg.data);
						//socket.emit('publish', rel);//{ subject, data, token });
						subscription = this.subscriptionsBySubject.get(subject);
						subscription.sockets.forEach((v, socket)=>{
							socket.emit('publish', rel)
						})
					...
					subscription = { subject, token, natsSubscription, sockets: new Map() };
					subscription.sockets.set(socket, 1)
					subscriptions.push(subscription);
					ctx.subscriptionTokenMap.set(token, subscription);
					this.subscriptionsBySubject.set(subject, subscription)
				...
				case 'request': {
					...
					this.nats.request(subject, jc.encode(data), { timeout : this.options.nats?.timeout || 30*1000 }).then((msg) => {
						const data = jc.decode(msg.data);
						const code  = "";//????
						const error = "";//????
						this.debug && 
						console.log("+++++++++++++++++++++++++++++++++++++++++++++++++++++++++");
						this.debug && 
						console.log('got response:'.brightRed, subject, '->', error, code, data);
	...
	subscriptionInfo(){
		//console.log("SUBSCRIPTION INFO: ".brightGreen);
		this.subscriptionMap.forEach(s => {
			//console.log("SUBSCRIPTION: ", s);
		})
		// this.subscriptionTokenMap.forEach(t => {
		// 	console.log("TOKEN: ", t);
		// })
